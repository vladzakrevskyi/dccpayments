...
(function ($) {
    ...
    jQuery(document).ready(function ($) {
        ...
        var form_partners = $('#contact-form-partners');
        var formMessages_partners = $('.form-message_partners');

        $(form_partners).submit(function (e) {
            e.preventDefault();
            var formData = form_partners.serialize();
            $.ajax({
                type: 'POST',
                url: form_partners.attr('action'),
                data: formData
            }).done(function (response) {
                // Make sure that the formMessages div has the 'success' class.
                $(formMessages_partners).removeClass('alert alert-danger');
                $(formMessages_partners).addClass('alert alert-success fade show');

                // Set the message text.
                formMessages_partners.html("<button type='button' class='close' data-dismiss='alert'>&times;</button>");
                formMessages_partners.append(response);

                // Clear the form.
                $('#contact-form-partners input,#contact-form-partners textarea').val('');
            }).fail(function (data) {
                // Make sure that the formMessages div has the 'error' class.
                $(formMessages_partners).removeClass('alert alert-success');
                $(formMessages_partners).addClass('alert alert-danger fade show');

                // Set the message text.
                if (data.responseText !== '') {
                    formMessages_partners.html("<button type='button' class='close' data-dismiss='alert'>&times;</button>");
                    formMessages_partners.append(data.responseText);
                } else {
                    $(formMessages_partners).text('Oops! An error occurred and your message could not be sent.');
                }
            });
        });
